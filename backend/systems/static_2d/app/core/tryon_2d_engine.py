...
def _headfit_scale_asset(
    ...
    if short_open_style and subject_hair_top_bbox is not None and cap_bbox is not None:
        scale = (subject_hair_top_bbox["width"] * 1.00) / max(cap_bbox["width"], 1)
    else:
        if subject_hair_bbox is not None:
            scales.append(subject_hair_bbox["width"] / max(asset_bbox["width"], 1))
        if subject_hair_top_bbox is not None and asset_top_bbox is not None:
            scales.append((subject_hair_top_bbox["width"] * 1.01) / max(asset_top_bbox["width"], 1))
        if subject_hair_top_bbox is not None and cap_bbox is not None:
            scales.append((subject_hair_top_bbox["width"] * 1.00) / max(cap_bbox["width"], 1))
    ...
    if subject_hair_bbox is not None and short_open_style:
        max_height_scale = (subject_hair_bbox["height"] * 1.12) / max(asset_bbox["height"], 1)
...
def _headfit_position(
    ...
    subject_opening = _legacy_forehead_opening_anchor(subject_hair_mask) if subject_hair_mask is not None else None
    asset_opening = _legacy_forehead_opening_anchor(asset_mask)

    if subject_opening is not None and asset_opening is not None:
        x = int(round(subject_opening[0] - asset_opening[0]))
        y = int(round(subject_opening[1] - asset_opening[1]))
    else:
        if subject_hair_top_bbox is not None and asset_top_bbox is not None:
    ...
    if _is_short_open_static_asset(asset):
        y -= max(int(face_bbox["height"] * 0.065), 12)
...
def _warp_short_open_asset_to_subject(
    input_image: Image.Image,
    face_analysis: FaceAnalysisResult,
    asset: AssetMetadata,
    asset_image: Image.Image,
...
def _legacy_compose_tryon(
    ...
    if subject_region is not None:
        ...
        if short_open_style and support_mask is not None:
            # Short, open men's styles have a forehead opening and lifted crown.
            # If we erase only where the new asset is opaque, the subject's
            # original hair keeps showing through the opening and we get the
            # narrow center-strip look. For these styles, erase the supported
            # subject-hair region behind the asset more decisively.
            erase_seed = np.clip(
                np.maximum(visible_asset, subject_hair_array * support_array * 0.98),
                0.0,
                1.0,
            )
            erase_blur = 3
...
def render_static_hybrid_tryon_image(
    ...
    subject_alpha = np.asarray(
        subject_region.filter(ImageFilter.MaxFilter(size=15)).filter(ImageFilter.GaussianBlur(radius=2)),
        dtype=np.float32,
    ) / 255.0
    ...
    width_top = max(int(top_ref["width"] * 1.12), 1)
    width_bottom = max(int(top_ref["width"] * 0.82), 1)
    ...
    if overlay_bbox is not None:
        crown_limit_y = overlay_bbox["y"] + int(overlay_bbox["height"] * 0.48)
        crown_limit_y = max(0, min(crown_limit_y, input_image.size[1]))
        if crown_limit_y < input_image.size[1]:
            extension_alpha[crown_limit_y:, :] *= 0.05
...
# Static non-live path uses subject-silhouette texture transfer by default.
# This remains the last stable baseline for the static notebook path.
render_static_tryon_image = render_headfit_static_tryon_image
render_static_tryon = render_headfit_static_tryon
