...
from support_app.core.tryon_2d_engine import (  # noqa: E402
    _apply_asset_mask,
    _average_point,
...
from support_app.models.schemas import AssetMetadata, FaceAnalysisResult  # noqa: E402
...
def _erase_subject_region(input_image: Image.Image, inpaint_mask: Image.Image | None) -> Image.Image:
...
def _build_prompt(asset: AssetMetadata) -> str:
    attrs = asset.normalized_attributes
    return (
        "Replace the subject hairstyle using the supplied reference hairstyle.\n"
        "Preserve face identity, pose, expression, skin tone, shoulders, clothing, and background.\n"
        "Edit only the hair and hairline region defined by the inpaint mask.\n"
        f"Target hairstyle attributes: length={attrs.length}, curl={attrs.curl}, "
        f"bang={attrs.bang}, volume={attrs.volume}, side_hair={attrs.side_hair}, "
        f"color={attrs.color}, style_family={attrs.style_family}.\n"
        "Match the reference silhouette, volume, bangs, and texture while keeping the result photorealistic."
    )


def prepare_generative_tryon_package(
    ...
    inpaint_mask = _build_inpaint_mask(input_image.size, face_analysis, subject_hair_mask)
    ...
    prompt_text = _build_prompt(asset)

    run_dir = PACKAGE_DIR / f"{input_stem}_{asset.asset_id}_{uuid4().hex[:8]}"
    ...
    prompt_out = run_dir / "generation_prompt.txt"
    prompt_out.write_text(prompt_text, encoding="utf-8")

    manifest = {
        "package_type": "generative_tryon_package",
        "subject": {
        ...
        "generation": {
            ...
            "task": "Use the reference hairstyle to generate a final edited hairstyle on the subject.",
            "preserve": [
                "face identity",
                "skin tone",
                "pose",
    ...
    manifest_out = run_dir / "package_manifest.json"
