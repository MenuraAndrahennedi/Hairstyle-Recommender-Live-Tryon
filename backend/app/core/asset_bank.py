...
from app.config import (
    ...
    PROCESSED_ASSET_METADATA_DIR,
    REVIEWED_ASSET_BANK_JSONL,
    REVIEWED_RENDER_SAFE_ASSET_BANK_JSONL,
)
from app.models.schemas import AssetMetadata
...
def _full_hair_row_to_asset_metadata(row: dict) -> AssetMetadata:
    normalized = row.get("labeling", {}).get("normalized_attributes", {})
...
def list_stage1_asset_metadata_paths() -> List[Path]:
    if not PROCESSED_ASSET_METADATA_DIR.exists():
        return []
    return sorted(PROCESSED_ASSET_METADATA_DIR.glob("*.json"))
...
def load_stage1_asset_bank() -> List[AssetMetadata]:
    assets: List[AssetMetadata] = []
    for path in list_stage1_asset_metadata_paths():
        payload = json.loads(path.read_text(encoding="utf-8"))
        assets.append(AssetMetadata.model_validate(payload))
    return assets
...
def load_full_hair_asset_bank() -> List[AssetMetadata]:
    ...
    for path in list_full_hair_asset_metadata_paths():
        payload = json.loads(path.read_text(encoding="utf-8"))
        assets.append(_full_hair_row_to_asset_metadata(payload))
    return assets
...
def load_reviewed_render_safe_asset_bank() -> List[AssetMetadata]:
    ...
    return [_full_hair_row_to_asset_metadata(row) for row in reviewed_rows]
...
def load_live_top_tier_asset_bank() -> List[AssetMetadata]:
    ...
    return [_full_hair_row_to_asset_metadata(row) for row in filtered_rows]
...
def load_asset_bank() -> List[AssetMetadata]:
    ...
    stage1_assets = load_stage1_asset_bank()
    if stage1_assets:
        return stage1_assets
...
def asset_bank_summary() -> dict:
    ...
    elif PROCESSED_ASSET_METADATA_DIR.exists():
        metadata_dir = str(PROCESSED_ASSET_METADATA_DIR)
    else:
